...
import { MaterialComponent } from "../GameObjects/Components/MaterialComponent";
...
export class Renderer {
    ...
    startRender = async (activeScene: Scene) => {
        ...
        mat4.perspective(projection, Math.PI / 4, 800 / 600, 0.1, 50000);
        ...
        for (const obj of gameObjects) {
            const materialComponent = obj.getComponent(MaterialComponent);
            const meshComponent = obj.getComponent(MeshComponent);

            if (!materialComponent) continue;
            if (!meshComponent) continue;

            if (!meshComponent?.mesh?.initialized!) await meshComponent.setupMesh();
            meshComponent.mesh.initialized = true;
            const mesh = meshComponent.getMesh();

            if (!mesh.buffer) continue;
            if (!materialComponent.material) continue;

            renderpass.setVertexBuffer(0, mesh.buffer); // mesh buffer
            renderpass.setBindGroup(1, materialComponent.material.bindGroup); // texture
            renderpass.draw(mesh.verticieCount, 1, 0, objectsDrawn);
            objectsDrawn++;
        }
